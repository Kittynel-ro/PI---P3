...
public class Settings implements Initializable {
    ...
    void savePref() throws SQLException {
        String query = "update ExamDates set partial_exam = '" + sem_start.getValue() + "',  final_exam = '" + sem_end.getValue() +
    ...
    private void loadPref() throws SQLException {
        checkConn();
        ResultSet rs = conn.createStatement().executeQuery("select * from ExamDates");
        while (rs.next()) {
            sem_start.setValue(LocalDate.parse(rs.getString("partial_exam")));
            sem_end.setValue(LocalDate.parse(rs.getString("final_exam")));
